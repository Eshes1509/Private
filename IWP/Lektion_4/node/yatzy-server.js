...
import {
    ...
    yatzyHighScorePage,
    ValidationError,
} from "./yatzy-site-forms-handling.js";
import { yatzyController } from "./yatzy-site-game-control.js";
...
function extractJSON(req) {
    ...
    else return Promise.reject(new Error(ValidationError));
}
        ...
        case "POST":
            {
                ...
                switch (pathElements[1]) {
                    case "newgame":
                        extractForm(req)
                            .then((gameData) =>
                                validateYatzyConfigData(gameData)
                            )
                            .then((gameData) =>
                                htmlResponse(res, newYatzyGamePage(gameData))
                            )
                    ...
                    case "nextround":
                        extractForm(req)
                            .then((roundData) =>
                                validateYatzyRoundData(roundData)
                            )
                            .then((roundData) =>
                                htmlResponse(res, newYatzyRoundPage(roundData))
                            )
                            .catch((err) => reportError(res, err));
                        break;
                    default:
                ...
                switch (
                    ...
                    case "highscores":
                        try {
                            ...
                            return htmlResponse(
                                res,
                                yatzyHighScorePage(validatedData)
                            );
                        } catch (err) {
                            reportError(res, err);
                        }
