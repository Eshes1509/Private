...
import {
    ...
    printHTMLHdr,
    printHTMLBody,
} from "./yatzy-site-game-control.js";
export {
    yatzyHomePage,
    newYatzyGamePage,
    newYatzyRoundPage,
    validateYatzyConfigData,
    validateYatzyRoundData,
    validateYatzyHighScoreData,
    yatzyHighScorePage,
    ValidationError,
    Validation,
};
...
function validateYatzyConfigData(gameFormData) {
    ...
    if (gameFormData.has("name") && gameFormData.has("diceCount")) {
        ...
        //and right values
        if (
            playerNameLen >= Validation.minNameLength &&
            playerNameLen <= Validation.maxNameLength &&
            diceCount >= Validation.minDiceCount &&
            diceCount <= Validation.maxDiceCount
        ) {
...
function validateYatzyRoundData(roundFormData) {
    ...
    if (roundFormData.has("gameID")) {
        ...
        if (game && !game.gameOver()) return roundData;
    }
...
function yatzyHighScorePage(validatedData) {
    //let fakePlayer = { navn: "Jens", score: 3000, diceNumber: 5 };

    let highscore = yatzyController.calculateHighScore();

    let yatzyHighScore = `

    <h1> IWP Multi Yatzy</h1>
  <p> Top scorer is: ${highscore.name} with ${highscore.score} points using ${highscore.diceCount} dice.</p>
  <a href="/">Start new game</a>
  `;

    let page = printHTMLHdr("IWP Yatzy Game", ["css/style.css"]);
    page += printHTMLBody(yatzyHighScore);
