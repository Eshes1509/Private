...
import { Validation, yatzyHighScorePage } from "./yatzy-site-forms-handling.js";
export { yatzyController, YatzyGame, printHTMLBody, printHTMLHdr };
...
let yatzyController = {
    ...
    hasGame: function (gameID) {
        return (
            gameID >= 0 && gameID < this.games.length && this.getGame(gameID)
        );
    },
    ...
    calculateHighScore: function () {
        let high = { name: "None", diceCount: 0, score: 0 };
        this.games.forEach((game) => {
            if (game.gameOver()) {
                if (getTotalScore(game.scoreTable) > high.score) {
                    high.score = getTotalScore(game.scoreTable);
                    high.name = game.name;
                    high.diceCount = game.diceCount;
                }
            }
        });
        return high;
...
function YatzyGame(gameID, name, diceCount) {
    ...
    this.gameOver = function () {
        //if all rounds have been played, stop the game
    ...
    <fieldset>
      ...
      <label for="difficultyLevel_id"> Difficulty Level:</label>
          <input
              ...
              required
          />
       <label for="top_down" name="play_direction">
           Play Top Down:</label
       >
       <input
           type="radio"
           ...
           required
       />
       <label for="bottom_up" name="play_direction">
           Play Bottom Up:</label
       >
       <input
           type="radio"
           id="bottom_up"
  ...
  * ******************************************************************************** */
  ...
  <input type="text" name="gameID" value="${
      this.gameID
  }" style="display:none" required minlength="1" maxlength="30">
    <input type="submit" value="Next Round" ${
        this.gameOn && !this.gameOver() ? "" : "disabled"
    }>
    </fieldset>
  </form>
  <div>
  ...
  <a href="/highscores">Show Highscore</a>`;
    ...
    this.printScoresHTML = function () {
        ...
        for (let round = 0; round < this.scoreTable.length; round++) {
            ...
            if (!isSpecialRound(round)) {
                ...
                for (
                    let d = 0;
                    d < this.scoreTable[round].diceRoll.length;
                    d++
                ) {
                    //BN: SOLUTION
            ...
            let clmn3 = printTableCellHTML(
                'class="right-text"',
                this.scoreTable[round].score
            );

            if (isSpecialRound(round))
                row += printTableRowHTML(
                    'class="row-fill"',
                    clmn1 + clmn2 + clmn3
                );
            else row += printTableRowHTML("", clmn1 + clmn2 + clmn3);
