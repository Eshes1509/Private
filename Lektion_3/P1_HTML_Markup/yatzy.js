...
const noRounds = 18;
const rounds = {
    //as C-enums doesn't directly exist in JS, we emulate it using an object
    ones: 0,
    twos: 1,
    threes: 2,
    fours: 3,
...
function isSpecialRound(round) {
    return (
        round === rounds.sum || round === rounds.bonus || round === rounds.total
    );
}
...
function countDice(diceRoll) {
    ...
    for (let i = 0; i < diceRoll.length; i++) {
        //foreach dice, increment the corresponding count entry
...
function printScores(scoreBoard) {
    for (let round = 0; round < scoreBoard.length; round++) {
        ...
        if (dices.length === 0) dices += paddString(M);

        let output = roundName;
        output +=
            paddString(15 - roundName.length) +
            "  " +
            dices +
            "  " +
            paddString(5 - score.length) +
            score;
        console.log(output);
    }
}
...
function calcIdentical(diceCount, N) {
    for (let dice = maxDice; dice >= minDice; dice--)
        if (diceCount[dice] >= N) return dice * N;
    return 0;
}
...
function calcTwoPairs(diceCount) {
    ...
    for (
        let d = maxDice;
        d >= minDice && noFoundPairs < 2;
        d-- //take greatest pairs by iterating backwards
    )
        if (diceCount[d] >= 2) {
            //a pair?
    ...
    if (noFoundPairs === 2)
        ...
        return pairScore[0] + pairScore[1];
    //total score
    else return 0;
}
...
function calcStraight(diceCount, start, stop) {
    ...
    for (let d = start; d <= stop; d++)
        if (diceCount[d] < 1) return 0;
        //a required dice is missing:stop!
        else sum += d;
    return sum;
}
...
function calcHouse(diceCount) {
    ...
    for (let d = maxDice; d >= minDice && twos === 0; d--)
        if (diceCount[d] >= 2 && d !== found3) {
            //skip position of three identical
            twos = d * 2;
        }
...
function calcYatzy(diceCount) {
    if (calcIdentical(diceCount, 5) > 0) return 50;
    //the normal score of a Yatzy: do you want the same in your multiYatzy?
    else return 0;
}
...
function printScoresHTML(scoreTable) {
    ...
    for (let round = 0; round < scoreTable.length; round++) {
        if (isSpecialRound(round)) res += '<tr class="row-fill">';
        else res += "<tr>";
